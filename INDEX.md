...
| `generate_dataset.py` | labelled synthetic generator, six planted patterns, deterministic | 1 |
...
- `data/synthetic_transactions.csv` / `synthetic_labels.csv` — the 229-tx labelled set (seed 7), regenerable.
- `tests/` — 60 tests: `python -m pytest`.

## Known-open, in one place

Real NTRO dataset not swapped in · GeoIP not derived · `layering` raw-flag
recall ~0.35 (feature + clustering + risk score do catch these; the IF flag
alone doesn't) · ~2 residual normal false positives on accidental long chains ·
no threshold tuning against held-out data. Details: `README.md` status list and
each night's "What is NOT done".
