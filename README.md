...
|---|---|---|
| **Labelled data** | `src/generate_dataset.py` | Synthesises transactions **and** a ground-truth label per wallet by planting six laundering patterns (peeling chain, high velocity, rapid movement, amount splitting, layering, circular flow). Deterministic per seed. A generator, not a detector — it's what lets the risk weights be *learned* instead of guessed. Each transaction's `src_ip` is drawn from a real public IP prefix and its `geo_country` / `asn` are then **looked up for real** against the bundled GeoIP database — not hand-assigned. Committed output: `data/synthetic_transactions.csv` (243 tx) + `data/synthetic_labels.csv` (349 entities, 69 anomalous). |
| **GeoIP enrichment** | `src/geoip.py` + `scripts/setup_geoip.py` | IP → country / ASN via the DB-IP Lite databases (CC BY 4.0), fetched once into `data/geoip/` and thereafter read as local `.mmdb` files — every lookup is an in-process b-tree read, no socket. `setup_geoip.py` needs internet; nothing else does. |
...
| **Clustering** | `src/clustering.py` | DBSCAN over the same features. Reports per-cluster purity: 4 clusters come out ≥80 % anomalous (57 of 69 anomalous wallets grouped by pattern), the 235-wallet mainstream blob 0 %. Supporting evidence, never a competing verdict. |
| **Risk (learned)** | `src/risk_model.py` | Four evidence signals per entity — AI anomaly score, graph evidence, behavioural evidence, cluster evidence — fed to a logistic regression fit against the labels. The fitted coefficients **are** the risk weights: `graph 0.36, cluster 0.30, ai 0.24, behavioral 0.10` — not a hand-picked 40/20/20/20. Risk is bucketed Low / Medium / High / Critical (prototype thresholds). |
...
| **Investigative lead** | `src/case_file.py` | Per flagged entity: risk + bucket, confidence + breakdown, why-flagged reasons, supporting TXIDs, related wallets/IPs, time-ordered timeline, `IP → TXID → Wallet → Wallet` investigation path, and a compact **per-lead subgraph spec**. |
| **Lead subgraph** | `src/subgraph.py` | Draws *only* the selected lead's money path — its wallets, relay IPs and the transactions between them — laid out left-to-right along the flow with amounts on the edges. A peeling chain shows its fan-out bursts; a circular flow shows a red arc returning to the start. Replaces the old full-dataset hairball as the evidence view. |
| **Dashboard** | `src/dashboard.py` | Minimal Streamlit: Command Center counts (+ the full monitored graph as a "scale" visual), ranked Priority Alerts, and per selected lead the case file beside its focused subgraph. `streamlit run src/dashboard.py`. |
...
- [x] **GeoIP enrichment** — `geo_country` / `asn` derived by a real lookup against the bundled DB-IP Lite database (`src/geoip.py`), offline
- [x] Offline self-check; `requirements-lock.txt` for a reproducible install
- [x] 73 tests (`python -m pytest`)
- [ ] **Real NTRO dataset** swapped in for the synthetic one
- [ ] **`layering` recall by the raw model flag is ~0.3** — the feature separates layering wallets cleanly and clustering + the risk score catch them, but the Isolation Forest itself under-flags long-chain interior wallets
- [ ] **False positives** — the Isolation Forest flags ~15 % of normal wallets and, on the current dataset, ~12 land in the High band (max normal risk ~76). The learned risk score's min-max scaling is sensitive to the dataset — across seeds this count swings between ~0 and ~14 — so the raw flag rate and the risk-band thresholds both still need tuning against held-out data
...
The GeoIP databases (`data/geoip/*.mmdb`) ship in the repo. To refresh them for a
newer month (needs internet, one time):

```bash
python scripts/setup_geoip.py              # or --month YYYY-MM
```
