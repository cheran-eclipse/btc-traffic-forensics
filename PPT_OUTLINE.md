...
Evidence: `requirements.txt`, `requirements-lock.txt` (pinned full closure).

### 7 — Data: labelled synthetic generator (spec 3b) + real GeoIP enrichment
Plants six laundering patterns and records a ground-truth label per wallet.
243 transactions, 349 entities (280 normal / 69 anomalous), deterministic.
Each `src_ip` is a real public address; `geo_country` / `asn` come from a real
lookup against the bundled DB-IP Lite database (`src/geoip.py`), not hand-typed.
Command: `python scripts/setup_geoip.py` then
`python src/generate_dataset.py --out-dir data --seed 7`
Evidence: `src/generate_dataset.py`, `src/geoip.py`; `data/synthetic_transactions.csv`,
`data/synthetic_labels.csv`; `data/geoip/*.mmdb` (CC BY 4.0); `NIGHT1_NOTES.md` §1.
...
including layering ~0.3 honestly).
...
DBSCAN over the same features. 4 clusters ≥80 % anomalous (57/69 grouped);
235-wallet mainstream blob 0 %. Supporting evidence, not a competing verdict.
...
`graph 0.36, cluster 0.30, ai 0.24, behavioral 0.10` vs naive `0.40/0.20/0.20/0.20`.
...
Command: `python src/case_file.py --wallets 1WYEQq1zGSyMaF46a5hN8yQnDx0340`
...
- Open: real NTRO dataset not yet swapped in; `layering` raw-flag recall ~0.3;
  risk-band thresholds dataset-sensitive (~12 normal wallets in High on the
  committed set, swings ~0–14 across seeds); thresholds untuned.
- Done since the nights: GeoIP enrichment — `geo_country`/`asn` now a real
  offline lookup (`src/geoip.py`, DB-IP Lite).
...
lead list, offline on seized data. Next: real dataset, threshold tuning on
held-out data, expand the dashboard's link analysis.
...
| 12 | case file | `python src/case_file.py --wallets 1WYEQq1zGSyMaF46a5hN8yQnDx0340` |
